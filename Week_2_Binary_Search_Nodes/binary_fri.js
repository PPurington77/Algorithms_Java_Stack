...
class BinarySearchTree {
     ...
     *///math.max() pass in two arg, returns which is larger
    height(node = this.root, counter = 0, val = []) { 
        if(node) {
            counter++
            val.push(counter)

            this.height(node.left, counter, val)
            this.height(node.right, counter, val)
        }
    ...
    isFull(node = this.root) { 
        ...
        //checks if both are not null and means the tree is full
